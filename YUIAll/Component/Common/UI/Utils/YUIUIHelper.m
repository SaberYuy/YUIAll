...
@implementation YUIUIHelper (QMUIMoreOperationAppearance)

+ (void)customMoreOperationAppearance {
    // 如果需要统一修改全局的 QMUIMoreOperationController 样式，在这里修改 appearance 的值即可
    [QMUIMoreOperationController appearance].cancelButtonTitleColor = UIColor.yui_tintColor;
}

@end


@implementation YUIUIHelper (QMUIAlertControllerAppearance)
...
@implementation YUIUIHelper (QMUIDialogViewControllerAppearance)
...
+ (void)customDialogViewControllerAppearance {
    ...
    appearance.backgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YUIThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YUIThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
        return UIColorWhite;
    }];
    appearance.headerViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YUIThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YUIThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
    ...
    appearance.headerSeparatorColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YUIThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YUIThemeIdentifierDark]) {
    ...
    buttonTitleAttributes[NSForegroundColorAttributeName] = UIColor.yui_tintColor;
    appearance.buttonTitleAttributes = [buttonTitleAttributes copy];
    
    appearance.buttonHighlightedBackgroundColor = [UIColor.yui_tintColor colorWithAlphaComponent:.25];
}

@end


@implementation YUIUIHelper (QMUIEmotionView)
...
+ (void)customEmotionViewAppearance {
    ...
    [QMUIEmotionView appearance].sendButtonBackgroundColor = UIColor.yui_tintColor;
}

@end

@implementation YUIUIHelper (QMUIImagePicker)
...
+ (void)customImagePickerAppearance {
    ...
    [QMUIImagePickerCollectionViewCell appearance].checkboxImage = [checkboxImage qmui_imageWithTintColor:UIColor.yui_tintColor];
    [QMUIImagePickerCollectionViewCell appearance].checkboxCheckedImage = [checkboxCheckedImage qmui_imageWithTintColor:UIColor.yui_tintColor];
    [QMUIImagePickerPreviewViewController appearance].toolBarTintColor = UIColor.yui_tintColor;
}

@end

@implementation YUIUIHelper (QMUIPopupContainerView)

+ (void)customPopupAppearance {
    QMUIPopupContainerView *popup = QMUIPopupContainerView.appearance;
    popup.backgroundColor = UIColor.yui_backgroundColor;
    popup.borderColor = UIColor.yui_separatorColor;
    popup.maskViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, __kindof NSObject<YUIThemeProtocol> * _Nullable theme) {
        return [identifier isEqual:YUIThemeIdentifierDark] ? UIColorMask : UIColorMaskWhite;
    }];
    
    QMUIPopupMenuView *menuView = QMUIPopupMenuView.appearance;
    menuView.itemSeparatorColor = UIColor.yui_separatorColor;
    menuView.sectionSeparatorColor = UIColor.yui_separatorColor;
    menuView.itemTitleColor = UIColor.yui_tintColor;
}

@end

@implementation YUIUIHelper (UITabBarItem)
...
@implementation YUIUIHelper (Button)
...
+ (QMUIButton *)generateDarkFilledButton {
    ...
    button.backgroundColor = UIColor.yui_tintColor;
    button.highlightedBackgroundColor = [UIColor.yui_tintColor qmui_transitionToColor:UIColorBlack progress:.15];// 高亮时的背景色
...
+ (QMUIButton *)generateLightBorderedButton {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(200, 40)];
    button.titleLabel.font = UIFontBoldMake(14);
    button.tintColorAdjustsTitleAndImage = UIColor.yui_tintColor;
    button.backgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, __kindof NSObject<NSCopying> * _Nullable identifier, __kindof NSObject * _Nullable theme) {
        return [UIColor.yui_tintColor qmui_transitionToColor:UIColor.yui_backgroundColor progress:.9];
    }];
    button.highlightedBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, __kindof NSObject<NSCopying> * _Nullable identifier, __kindof NSObject * _Nullable theme) {
        return [UIColor.yui_tintColor qmui_transitionToColor:UIColor.yui_backgroundColor progress:.75];
    }];// 高亮时的背景色
    button.layer.borderColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, __kindof NSObject<NSCopying> * _Nullable identifier, __kindof NSObject * _Nullable theme) {
        return [button.backgroundColor qmui_transitionToColor:UIColor.yui_tintColor progress:.5];
    }].CGColor;
    ...
    button.highlightedBorderColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, __kindof NSObject<NSCopying> * _Nullable identifier, __kindof NSObject * _Nullable theme) {
        return [button.backgroundColor qmui_transitionToColor:UIColor.yui_tintColor progress:.9];
    }];// 高亮时的边框颜色
    return button;
}
...
@implementation YUIUIHelper (Emotion)
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (QMUIEmotion *e in emotions) {
            e.image = [UIImageMake(e.identifier) qmui_imageWithBlendColor:UIColor.yui_tintColor];
        }
    });
}
...
@implementation YUIUIHelper (SavePhoto)
...
@implementation YUIUIHelper (Calculate)
...
@implementation YUIUIHelper (Theme)
...
@implementation YUIUIHelper (Debug)
...
+ (QMUIInteractiveDebugPanelViewController *)generateDebugViewControllerWithTitle:(NSString *)title items:(NSArray<QMUIInteractiveDebugPanelItem *> *)items {
    ...
    vc.styleConfiguration = ^(QMUIInteractiveDebugPanelViewController * _Nonnull viewController) {
        viewController.view.backgroundColor = UIColor.yui_backgroundColorLighten;
        viewController.view.layer.borderColor = UIColor.yui_separatorColor.CGColor;
        viewController.titleLabel.textColor = UIColor.yui_titleTextColor;
        [viewController.debugItems enumerateObjectsUsingBlock:^(QMUIInteractiveDebugPanelItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.titleLabel.textColor = UIColor.yui_titleTextColor;
            obj.actionView.tintColor = UIColor.yui_tintColor;
            if ([obj.actionView isKindOfClass:UITextField.class]) {
                ((UITextField *)obj.actionView).textColor = UIColor.yui_titleTextColor;
                ((UITextField *)obj.actionView).qmui_borderColor = UIColor.yui_separatorColor;
            }
        }];
    };
    return vc;
