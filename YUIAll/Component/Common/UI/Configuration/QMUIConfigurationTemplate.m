...
#pragma mark - Global Color
    ...
    QMUICMI.grayDarkenColor = UIColor.yui_mainTextColor;                          // UIColorGrayDarken : 深一点的灰色
    ...
    QMUICMI.backgroundColor = UIColor.yui_backgroundColor;                                     // UIColorForBackground : 界面背景色，默认用于 QMUICommonViewController.view 的背景色
    ...
    QMUICMI.separatorColor = UIColor.yui_separatorColor;                          // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUIBorder) 分隔线颜色
    QMUICMI.separatorDashedColor = UIColorMake(17, 17, 17);                     // UIColorSeparatorDashed : 全局默认的虚线分隔线的颜色，默认 QMUIKit 暂时没用到
    QMUICMI.placeholderColor = UIColor.yui_placeholderColor;                                    // UIColorPlaceholder，全局的输入框的 placeholder 颜色，默认用于 QMUITextField、QMUITextView，不影响系统 UIKit 的输入框
...
#pragma mark - UIButton
    ...
    QMUICMI.buttonTintColor = UIColor.yui_tintColor;                              // ButtonTintColor : QMUIButton 默认的 tintColor，不影响系统的 UIButton
    
    
#pragma mark - TextInput
    QMUICMI.textFieldTextColor = UIColor.yui_titleTextColor;                     // TextFieldTextColor : QMUITextField、QMUITextView 的 textColor，不影响 UIKit 的输入框
    QMUICMI.textFieldTintColor = UIColor.yui_tintColor;                          // TextFieldTintColor : QMUITextField、QMUITextView 的 tintColor，不影响 UIKit 的输入框
...
#pragma mark - UISwitch
    QMUICMI.switchOnTintColor = UIColor.yui_tintColor;                           // SwitchOnTintColor : UISwitch 打开时的背景色（除了圆点外的其他颜色）
    QMUICMI.switchOffTintColor =  UIColor.yui_separatorColor;                    // SwitchOffTintColor : UISwitch 关闭时的背景色（除了圆点外的其他颜色）
...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarBackgroundImage = UIImage.yui_navigationBarBackgroundImage;    // NavBarBackgroundImage : UINavigationBar 的背景图，注意 navigationBar 的高度会受多个因素（是否全面屏、是否使用了 navigationItem.prompt、是否将 UISearchBar 作为 titleView）的影响，要检查各种情况是否都显示正常。
    if (@available(iOS 15.0, *)) {
    ...
    QMUICMI.navBarBackIndicatorImage = UIImage.yui_navigationBarBackIndicatorImage;                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片，图片尺寸建议为(13, 21)，否则最终的图片位置无法与系统原生的位置保持一致
    QMUICMI.navBarCloseButtonImage = UIImage.yui_navigationBarCloseImage;     // NavBarCloseButtonImage : QMUINavigationButton 用到的 × 的按钮图片
    ...
    QMUICMI.navBarAccessoryViewTypeDisclosureIndicatorImage = UIImage.yui_navigationBarDisclosureIndicatorImage;     // NavBarAccessoryViewTypeDisclosureIndicatorImage : QMUINavigationTitleView 右边箭头的图片
...
#pragma mark - TabBar
    ...
    QMUICMI.tabBarItemTitleColor = UIColor.yui_descriptionTextColor;             // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemTitleColorSelected = UIColor.yui_tintColor;                // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
...
#pragma mark - Toolbar
    ...
    QMUICMI.toolBarTintColor = UIColor.yui_tintColor;                            // ToolBarTintColor : NavBarContainerClasses 里的 UIToolbar 的 tintColor，以及 QMUIToolbarButton normal 状态下的文字颜色
...
#pragma mark - SearchBar
    ...
    QMUICMI.searchBarTextFieldBackgroundImage = UIImage.yui_searchBarTextFieldBackgroundImage;       // SearchBarTextFieldBackgroundImage : QMUISearchBar 里的文本框的背景图，图片高度会决定输入框的高度
    ...
    QMUICMI.searchBarBackgroundImage = UIImage.yui_searchBarBackgroundImage;     // SearchBarBackgroundImage : 搜索框的背景图，如果需要设置底部分隔线的颜色也请绘制到图片里
    QMUICMI.searchBarTintColor = UIColor.yui_tintColor;                          // SearchBarTintColor : QMUISearchBar 的 tintColor，也即上面的操作控件的主题色
    QMUICMI.searchBarTextColor = UIColor.yui_titleTextColor;                                  // SearchBarTextColor : QMUISearchBar 里的文本框的文字颜色
    QMUICMI.searchBarPlaceholderColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YUIThemeProtocol> *theme) {
        if ([identifier isEqualToString:YUIThemeIdentifierDark]) {
...
#pragma mark - Plain TableView
    ...
    QMUICMI.tableViewCellTitleLabelColor = UIColor.yui_mainTextColor;             // TableViewCellTitleLabelColor : QMUITableViewCell 的 textLabel 的文字颜色
    QMUICMI.tableViewCellDetailLabelColor = UIColor.yui_descriptionTextColor;     // TableViewCellDetailLabelColor : QMUITableViewCell 的 detailTextLabel 的文字颜色
    QMUICMI.tableViewCellBackgroundColor = UIColorForBackground;                 // TableViewCellBackgroundColor : QMUITableViewCell 的背景色
    QMUICMI.tableViewCellSelectedBackgroundColor = UIColor.yui_backgroundColorHighlighted;  // TableViewCellSelectedBackgroundColor : QMUITableViewCell 点击时的背景色
    ...
    QMUICMI.tableViewCellDisclosureIndicatorImage = UIImage.yui_tableViewCellDisclosureIndicatorImage;    // TableViewCellDisclosureIndicatorImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryDisclosureIndicator 时的箭头的图片
    QMUICMI.tableViewCellCheckmarkImage = UIImage.yui_tableViewCellCheckmarkImage;  // TableViewCellCheckmarkImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryCheckmark 时的打钩的图片
    QMUICMI.tableViewCellDetailButtonImage = UIImage.yui_tableViewCellDetailButtonImage; // TableViewCellDetailButtonImage : QMUITableViewCell 当 accessoryType 为 UITableViewCellAccessoryDetailButton 或 UITableViewCellAccessoryDetailDisclosureButton 时右边的 i 按钮图片
    ...
    QMUICMI.tableViewSectionHeaderBackgroundColor = UIColor.yui_separatorColor;                         // TableViewSectionHeaderBackgroundColor : Plain 类型的 QMUITableView sectionHeader 的背景色
    QMUICMI.tableViewSectionFooterBackgroundColor = UIColor.yui_separatorColor;                         // TableViewSectionFooterBackgroundColor : Plain 类型的 QMUITableView sectionFooter 的背景色
...
#pragma mark - Grouped TableView
    ...
    QMUICMI.tableViewGroupedBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YUIThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YUIThemeIdentifierDark]) {
    ...
    QMUICMI.tableViewGroupedCellBackgroundColor = UIColor.yui_backgroundColorLighten;                    // TableViewGroupedCellBackgroundColor : Grouped 类型的 QMUITableView cell 背景色
...
- (BOOL)shouldApplyTemplateAutomatically {
    ...
    NSString *selectedThemeIdentifier = [[NSUserDefaults standardUserDefaults] stringForKey:YUISelectedThemeIdentifier];
