...
#import "YACommonGridViewController.h"

@interface YACommonGridButton : QMUIButton

@end

@implementation YACommonGridViewController
...
- (YACommonGridButton *)generateButtonAtIndex:(NSInteger)index {
    NSString *keyName = self.dataSource.allKeys[index];
    NSAttributedString *attributedString = [[NSAttributedString alloc] initWithString:keyName attributes:@{NSForegroundColorAttributeName: UIColor.ya_descriptionTextColor, NSFontAttributeName: UIFontMake(11), NSParagraphStyleAttributeName: [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:12 lineBreakMode:NSLineBreakByTruncatingTail textAlignment:NSTextAlignmentCenter]}];
    UIImage *image = (UIImage *)self.dataSource[keyName];
    image = [image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    
    YACommonGridButton *button = [[YACommonGridButton alloc] init];
    button.tintColor = UIColor.ya_gridItemTintColor;
...
- (void)handleGridButtonEvent:(YACommonGridButton *)button {
...
@implementation YACommonGridViewController (UISubclassingHooks)
...
@implementation YACommonGridButton
